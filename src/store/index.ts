...
interface AppState {
  ...
  completeProcessing: (status?: 'completed' | 'error') => void;
...
function stripLargeData(task: BatchTask): BatchTask {
  return {
    ...task,
    // 移除文件的 base64 / Blob 引用数据，只保留元数据
    files: task.files.map((f) => ({
      ...f,
      url: '', // 清空 base64 URL
      preview: '', // 清空预览数据
      originalDataUrl: '', // 清空原始 base64
      blobKey: '', // 清空 IndexedDB 引用
    })),
    // 移除结果中的 URL 数据
...
export const useAppStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      // 添加图像（F-08：上传即选中，新图 id 并入 selectedImageIds）
      addImages: (files) => {
        set((state) => ({
      ...
      // 移除图像（同步清理 IndexedDB 中的原始 Blob）
      removeImages: (ids) => {
        const removed = get().images.filter((img) => ids.includes(img.id));
        removed.forEach((img) => {
          if (img.blobKey) {
            void deleteBlob(img.blobKey);
          }
        });
        set((state) => ({
      ...
      // 完成处理（F-14：支持中止/失败时以 error 状态 + 实际进度入库，而非 100%）
      completeProcessing: (status: 'completed' | 'error' = 'completed') => {
        set((state) => {
          if (!state.currentTask) return state;
          ...
          // 限制历史记录数量
          const newHistory = [completedTask, ...state.taskHistory].slice(
            0,
            MAX_HISTORY_ITEMS
          );
    ...
    }),
    {
      ...
      // 只持久化必要的配置和精简后的历史
      // 注意：images 不入持久化；taskHistory 已通过 stripLargeData 清除 blobKey/originalDataUrl
      partialize: (state) => ({
...
export const useIsProcessing = () =>
  useAppStore((state) => state.isProcessing);
