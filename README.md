...
**专业图像处理平台 — 批量处理 · 格式转换 · 滤镜特效 · 水印 · 智能去重**

一个完全本地运行、开箱即用的现代图像处理工作台。无需注册、无需上传到云端，所有图片都在你自己的机器上通过 Sharp 引擎高速处理。
...
[English](README_EN.md) · [功能特性](#-功能特性) · [快速开始](#-快速开始) · [技术栈](#-技术栈) · [项目结构](#-项目结构) · [安全特性](#-安全特性)
...
| **尺寸调整** | 智能缩放，支持适应 / 填充 / 拉伸 / 裁剪模式 | - |
| **智能裁剪** | 自定义裁剪区域，支持 1:1、16:9、4:3、3:2 等预设比例 | - |
| **旋转翻转** | 任意角度旋转，垂直 / 水平翻转（翻转真实生效） | - |
| **滤镜效果** | 15+ 专业滤镜：灰度、复古、锐化、模糊、浮雕等 | - |
| **色彩调整** | 亮度 / 对比度 / 饱和度精细调节 | - |
| **水印添加** | 文字 / 图片水印，9 宫格定位 + 平铺模式（支持旋转与间距） | - |
| **边框装饰** | 自定义边框宽度、颜色、圆角 | - |
| **保留元数据** | 处理时保留 EXIF / ICC 元数据（可开关） | - |
| **目标压缩** | 按目标大小（KB）二分逼近压缩，适合 Web 优化 | JPEG, WebP |

### 🧠 高级功能

- **批量处理** — 一次处理数百张图片，支持并发控制（1-10 路）与错误中断选项
- **处理方案** — 预设方案一键应用（Web 优化 / 水印保护等），支持自定义方案保存 / 导入 / 导出（导入带运行时校验）
- **智能去重** — 感知哈希算法识别相似 / 重复图片，支持相似度阈值配置，返回真实相似度
...
- **批量下载** — 一键打包 ZIP 下载，重命名模板支持 `{counter}` 防覆盖
...
| 单文件大小上限 | 50 MB（`file-validation.ts` 中的 `MAX_FILE_SIZE`） |
| 支持格式 | JPEG / PNG / WebP / GIF（拒绝 SVG / PDF 等矢量，防止攻击面） |
...
- **限流加固** — 基于真实连接 IP（不信任可伪造转发头），存储容量上限
...
4. **提交**：`git commit` 时遵循 Conventional Commits 风格（`feat:` / `fix:` / `refactor:` 等）
5. **发起 PR**：描述改动内容与验证结果

> 💡 新手友好：`src/lib/` 下的纯函数（file-validation、config-schema）是很好的首个贡献切入点。
...
<p align="center">
  由 <a href="https://github.com/riceshowerX">riceshowerX</a> ❤️ 构建 · 本地优先 · 隐私至上
</p>
